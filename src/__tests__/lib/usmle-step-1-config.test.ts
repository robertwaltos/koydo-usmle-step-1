...
import { EXAM_CONFIG } from "@/lib/usmle-step-1/config";

describe("USMLE Step 1 Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM133");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("usmle-step-1");
  });
