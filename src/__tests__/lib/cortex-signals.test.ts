...
import { emitPrismSignal } from "@/lib/usmle-step-1/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "usmle-step-1" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "usmle-step-1" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "usmle-step-1" });
